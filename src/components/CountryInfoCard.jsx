...
export default function CountryInfoCard() {
  ...
  const { countryInfo, neighbours, setCountryName } = useContext(CountryContext);
  const country = countryInfo[0];

  const { isNewCountry, setIsNewCountry, setLatLng } = useContext(NewCountryContext);
  ...
  const giniInd = country.gini ? country?.gini[Object.keys(country?.gini).at(-1)] : null;
  ...
  if (country.timezones[0].includes('+')) {
    time = DateTime.local().setZone(Number.isFinite(timezone) ? `UTC+${timezone + 1}` : 'UTC');
  ...
  const handleCountryClick = (neigbour) => {
    ...
    map.panTo([neigbour.capitalInfo.latlng[0] - 0.17, neigbour.capitalInfo.latlng[1] + 0.5]);
  };

  return (
    <div className='CountryInfoCard'>
      <img className='country__flag' src={flag} alt={`Flag of ${officialName}`} />
      ...
      <ul className='country__facts'>
        ...
        {giniInd && <li>Gini Index: {giniInd} (the lower the more equality)</li>}
        ...
        <li>
          ...
          {neighbours.length === 0
            ...
            : neighbours?.map((neigbour) => (
                ...
                >
                  <img className='country__neighbour-flag' src={neigbour?.flags.svg} alt={neigbour?.name.common} />
                  <div className='hover' onClick={() => setCountryName(neigbour?.name.common)}>
